...
#def calculate_team_efficiency(team_projects):
 #   completed_projects = 0
  #  total_projects = len(team_projects)
   ...
   # for project in team_projects:
    #    risk_level = project["Risk Level"].lower()
...
def calculate_team_efficiency(team_projects):
    ...
    for project in team_projects:
        if isinstance(project, dict):  # Check if the project is a dictionary
...
# Function to load data from JSON files in the current directory
def load_data_from_json_folder():
...
loaded_data = load_data_from_json_folder()
